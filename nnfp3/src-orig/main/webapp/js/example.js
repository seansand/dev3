...
function onDocumentLoad()
{
   ...
   //displayCurrentSums(playerScoreArray, resultsMap.unknown);
   ...
   if (resultsMap.unknown.length > 0)
   {
      ...
      displayNflProjections(projections, mflObj);   
...
function displayKeyMatchups(projections, mfl)
{
   ...
   for (var i = 0; i < keyMatchupsArray.length; ++i)
   {
      ...
      for (var j = 0; j < Math.min(10, matchesLeft); ++j)   //display no more than 10
      {
...
function displayNflProjections(projections, mfl)
{
   var nflString = 
   ...
   for (team in winners)
   {
      if (winners[team] / reps > 0.5 || (winners[team] / reps == 0.5 && mfl[team].home == 1))
      {
         ...
         var tdString = mfl[team].remaining >= 3600 || mfl[team].remaining <= 0 ?
            "<TD ateamn=" + awayTeam + " align=center bgcolor='#F0F0F0'>" :          //not in progress gray
            "<TD ateamn=" + awayTeam + " align=center bgcolor='#FFFFEE'>";           //in progress yellow
         
         nflStringArray[nflStringArray.length] = tdString +
         displayNflScores(awayTeam, awayScore, homeTeam, homeScore, mfl[team].remaining) + "<BR>" +
...
function displayProjections(projections)
{
   var projectionString = 
      "<P>Projections below are based on point spreads and the scores of games in progress. " +
   ...
   $.map(projections.playerWinsArray, function(item) {
      ...
      projectionString += "<BR>" + "<SPAN class='projection' id = '" + player + "proj'>" 
                          + proj + "</SPAN>";
...
function displayCurrentScores(playerScoreArray, unknownTeams)
{
   ...
   //if (unknownTeams.length == 0)   //only display this when finished
   {
      ...
      $.map(playerScoreArray, function(playerScorePair) {
         resultsString += getOnlyKey(playerScorePair) + ": <B>" + getOnlyVal(playerScorePair) + "</B><span class='bigfont'>&nbsp;</span><BR>";
      });
   }

...
<p id="picks">
   <%
      Integer year = (new Date() - 180).getAt(Calendar.YEAR)  
      def p = java.util.regex.Pattern.compile("\\((\\d+)\\)");
      def m = p.matcher(request.picksTxt);
      if (m.find())
      {
         print('<h2 id="nnfpbetaheader" class="betaheader"><i>NNFP latest results</I>&nbsp;&nbsp;<SMALL><SMALL><SPAN title=' + year + '>WEEK ' + m.group(1) + '</SPAN></SMALL></SMALL>');
         print('</h2>');
      }
